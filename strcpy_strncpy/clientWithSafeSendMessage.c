...
int main( int argc, char *argv[] ){
	...
	int i = 0;
	if ( argc == 3 ) {
		/* First copy first 7 characters of recipients name using strncpy
		 */
		strncpy(friendsName,argv[1], 7); // copy will go from 0th to 6th character
		
		/* Then null terminate it with te end of string character. */ 
		friendsName[7] = '\0'; 
	
		/* Also copy first 19 characters or the message */
		strncpy(message, argv[2], 19); // 0th to 1th character
